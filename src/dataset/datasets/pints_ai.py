...
class PintsAIDataset(BaseDataset):
    ...
    def _run_post_processing(self) -> None:
        ...
        cache_dir_path: str = self.get_segment_cache_dir_path(self.global_cfg.model.max_length, 0)
        if os.path.exists(cache_dir_path):
            # Load the cached segments
            log_if_rank_zero(
                logger, f"Loading cached segments from {cache_dir_path}"
            )
            dataset_of_segments = Dataset.load_from_disk(cache_dir_path)
        else:
            ...
            should_disable_tqdm = torch.distributed.is_initialized() and torch.distributed.get_rank() != 0
            for example in tqdm.tqdm(dataset, desc="Segmenting data", disable=should_disable_tqdm):
            ...
            log_if_rank_zero(logger, f"Saving {len(dataset_of_segments)} segmented data to {cache_dir_path}")
            dataset_of_segments.save_to_disk(cache_dir_path)
