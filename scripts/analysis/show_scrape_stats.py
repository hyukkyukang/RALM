...
def main(config_dir_path: str, data_dir_path: str) -> None:
    ...
    total_count = postgres_connector.execute_and_fetchall("SELECT count(*) from metadata")[0][0]
    success_count = postgres_connector.execute_and_fetchall("SELECT count(*) from metadata WHERE success = TRUE")[0][0]
    failed_count = postgres_connector.execute_and_fetchall("SELECT count(*) from metadata WHERE success = FALSE")[0][0]
        
    print(f"Total count: {total_count}")
    print(f"Success count: {success_count}")
    print(f"Failed count: {failed_count}")
    
    assert total_count == (success_count + failed_count), "Total count does not match success and failed count"
    ...
    print(f"Disk memory usage of {data_dir_path}: {file_utils.bytes_to_readable(dir_logical_size)}")
    print(f"Disk disk usage of {data_dir_path}: {file_utils.bytes_to_readable(dir_physical_size)}")
    return None
