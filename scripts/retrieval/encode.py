...
from src.retrieval import SentenceTransformerCorpusEncoder
from src.utils import get_ip, get_partition_indices
...
def process_partition(rank: int, world_size: int, cfg: DictConfig) -> None:
    ...
    worker_start_idx_of_current_server = (
        cfg.retrieval.encoding.worker_start_idx_of_current_server
    )
    ...
    save_dir: str = os.path.join(
        cfg.retrieval.embedding_dir_path, server_ip, f"gpu_{rank}"
    )
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    # Instantiate the encoder and move the model to the designated GPU.
    encoder = SentenceTransformerCorpusEncoder(
