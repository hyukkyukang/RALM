...
from src.retrieval.single_vector_retrieval.retriever import SentenceTransformerRetriever
...
def fix_retrieval_result(
    ...
    passage_id, local_chunk_start_idx, local_chunk_end_idx = (
        convert_global_chunk_id_to_passage_id_and_local_chunk_range(
            global_chunk_id, retriever.num_chunks_per_passage, retriever.chunk_size
        )
    )

    # Get the query chunk for the retrieval
    query_chunk = dataset[passage_id]["input_ids"][
        local_chunk_start_idx:local_chunk_end_idx
    ]
    query_str = tokenizer.decode(query_chunk, skip_special_tokens=True)

    # Perform the retrieval
    retrieved_global_chunk_ids: List[int] = retriever.search(
        query_str, k=topk, passage_id_to_ignore=passage_id
    )
    assert (
        len(retrieved_global_chunk_ids) == topk
    ), f"The number of retrieved global chunk ids is not equal to the topk: {len(retrieved_global_chunk_ids)} != {topk}"
    assert (
        -1 not in retrieved_global_chunk_ids
    ), f"The global chunk id {global_chunk_id} is not in the retrieved chunk ids"

    # Print the retrieved global chunk ids
    retrieved_texts: List[str] = [
        retriever.convert_global_chunk_id_to_text(global_chunk_id)
        for global_chunk_id in retrieved_global_chunk_ids
    ]
    ...
    shard_idx, shard_local_idx = (
        retrieved_chunk_dataset.global_chunk_id_to_shard_idx_and_local_idx(
            global_chunk_id
        )
    )
...
def main(cfg: DictConfig) -> None:
    ...
    retrieved_chunk_dataset = RetrievedChunkDataset(
        dataset_name=data_module.train_dataset.name, cfg=cfg.retrieval, global_cfg=cfg
    )
    ...
    for idx, target_global_chunk_id in enumerate(
        tqdm.tqdm(target_global_chunk_ids, desc="Fixing retrieval result")
    ):
    ...
    process_func = partial(
        modify_shard_process,
        modifications_by_shard=modifications_by_shard,
        retrieved_chunk_dataset=retrieved_chunk_dataset,
    )
    ...
    with Pool(processes=len(shard_indices)) as pool:
        results = list(
            tqdm.tqdm(
                pool.imap(process_func, shard_indices),
