...
from scripts.analysis.attention import (
    custom_causal_retrieval_block_attention, get_causal_retrieval_block_mask)
...
def get_projection_matrices(
    ...
    query_projection_matrix = torch.randn(1, 12, 64, 64, device=device)  # Changed dimensions
    key_projection_matrix = torch.randn(1, 12, 64, 64, device=device)    # Changed dimensions
    value_projection_matrix = torch.randn(1, 12, 64, 64, device=device)  # Changed dimensions
...
def encode_input_and_retrieval_vectors(
    ...
    query_projection_matrix, key_projection_matrix, value_projection_matrix = get_projection_matrices(
        token_ids_length,
        retrieval_block_num,
    ...
    key = key.reshape(1, 3, 4, -1, 64).mean(dim=2)  # Reshape and mean to go from 12->3 heads

    value = kv_vector @ value_projection_matrix
    value = value.reshape(1, 3, 4, -1, 64).mean(dim=2)  # Reshape and mean to go from 12->3 heads
...
def main():
    ...
    output1 = flex_attention(query, key, value, block_mask=causal_retrieval_block_mask, scale=scale, enable_gqa=True, return_lse=False)
    ...
    output2 = flex_attention(query, key, value, scale=scale, block_mask=causal_retrieval_block_mask, enable_gqa=True, return_lse=False)

    # Check causality
    assert torch.allclose(output1[:, :, :target_position, :], 
                        output2[:, :, :target_position, :], 
                        atol=TOLERANCE), "Causality violated: changes affected previous positions"

    # Check that EVERY position after target_position is different
    not_affected = []
    for pos in tqdm(range(target_position, output1.shape[2]), desc="Checking positions"):
        if torch.allclose(
            output1[:, :, pos, :],
            output2[:, :, pos, :],
            atol=TOLERANCE
